...
from eval_helpers import get_eval_single, auc_result_summary
...
def print_summary(data):
    ...
    print("total n (before splitting etc): ", len(data))
...
if __name__ == '__main__':
    ...
    for i in range(int(args.trials)):
        ...
        if i == 0: # only print information if it's the first run
            probs = get_data(args.data, seed=0, verb=True, algo=args.algo)
            print_summary(probs)
        else:
            probs = get_data(args.data, seed=i, verb=False, algo=args.algo)
